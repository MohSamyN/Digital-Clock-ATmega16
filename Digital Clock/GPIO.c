...
/*Port Control*/
#define PORT_R(DATA,MASK,PORT_ID)		*((GPIO_RegAddType)(PortBaseAddressLut[PORT_ID])) = (*((GPIO_RegAddType)(PortBaseAddressLut[PORT_ID])) & ~(MASK)) | (DATA & MASK)
#define DDR_R(PORT_ID)					*((GPIO_RegAddType)(DDRBaseAddressLut[PORT_ID])) 
#define PIN_R(MASK,PORT_ID)				*((GPIO_RegAddType)(PinBaseAddressLut[PORT_ID])) & MASK
